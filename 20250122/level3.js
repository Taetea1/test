...
const getUserInfo = () => {
  ...
  if (userList === null) {
    ...
    data.push(...userList);
    data.map((x, i) => {
      let index = userList.findIndex((obj) => obj.id === `${x.id}`);
      tbodys.innerHTML += `
    <tr>
      ...
      <button id="${index}${x.id}" class="changebtn" onclick=changeData(${x.id})>수정</button><button class="changebtn" onclick=removeData(${x.id})>삭제</button>
      </div></td>
    </tr>
  `;
    });
...
btn.addEventListener("click", () => {
  if (
    ...
    let index = userList.findIndex(
      (obj) => obj.id === `${data[data.length - 1].id}`
    );
    tbodys.innerHTML += `
    <tr>
...
const removeData = (id) => {
  ...
  let index = userLists.findIndex((obj) => obj.id === `${x.id}`);
  data = [];
  ...
  data.map((x, i) => {
    ...
    <tr>
      ...
      <td class="test${x.id}">${x.car}</td>
      <td>${x.nick}</td>
      <td><div class="flexbtn"><button id="${index}${x.id}" class="changebtn" onclick=changeData(${x.id})>수정</button><button class="changebtn" onclick=removeData(${x.id})>삭제</button></div></td>
    </tr>
  `;
  });
...
const changeData = (id) => {
  ...
  let changecar2 = JSON.parse(localStorage.getItem("userInfo"));
  let index2 = changecar2.findIndex((obj) => obj.id === `${id}`);
  btnsss = document.getElementById(`${index2}${id}`);
  if (btnsss.innerText === "수정") {
    btnsss.innerText = "수정완료";
    inputtest3.innerHTML = `
    <input class="c c3${id}" value=${inputtest3.innerText} oninput=checkChangename() />`;
    inputtest2.innerHTML = `
    <input type=number maxlength=3 oninput=numberMaxLength2(this) class="c c2${id}" value=${inputtest2.innerText} />
    <div class=changecar2></div>`;
    inputtest.innerHTML = `
    <input class="c c${id}" value=${inputtest.innerText} oninput=checkChangeCar() />
...
const numberMaxLength2 = (e) => {
  ...
  const ageEle = document.querySelector(".changecar2");

  if (c2.value >= 150 || c2.value < 0) {
