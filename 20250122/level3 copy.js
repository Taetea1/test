...
const changeData = (id) => {
  ...
  if (btnsss.innerText === "수정") {
    ...
    <input class="c c3${id}" value=${inputtest3.innerText} oninput=checkChangename(${id}) />`;
    inputtest2.innerHTML = `
    <input type=number maxlength=3 oninput=numberMaxLength2(this) class="c c2${id}" value=${inputtest2.innerText} />
    <div class="changecar2${id}"></div>`;
    inputtest.innerHTML = `
    <input class="c c${id}" value=${inputtest.innerText} oninput=checkChangeCar(${id}) />
    <div class="changecar${id}"></div>`;
